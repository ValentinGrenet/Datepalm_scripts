...
# Load table
tg <- read.csv('/home/valentin-grenet/Bureau/Données/TE_sequences/Phoenix_dactylifera.LTR_ages.csv', header=T)
...
for (te in consensi){ 
  ...
  leaf_ages$y[which(leaf_ages$consensus=='consensus_Cluster_0')] <- 1
  leaf_ages$my <- leaf_ages$branch.length/0.013
  ...
  geom1 <- MASS::fitdistr(leaf_ages$branch.length, "geometric")
  geom1$loglik <- sum(stats::dgeom(leaf_ages$my, geom1[["estimate"]], 
                                     log = TRUE))
